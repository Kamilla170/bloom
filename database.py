...
class PlantDatabase:
    ...
    async def update_plant_state(self, plant_id: int, user_id: int, new_state: str, 
        ...
        async with self.pool.acquire() as conn:
            ...
            # Если есть корректировка полива, обновляем интервал
    ...
    async def update_user_reminder_settings(self, user_id: int, reminder_time: str = None, 
        ...
        async with self.pool.acquire() as conn:
            ...
            if updates:
                ...
                """
                await conn.execute(query, *params)import os
import asyncpg
import json
from datetime import datetime, timedelta
from typing import Dict, List, Optional
...
class PlantDatabase:
    ...
    async def update_qa_feedback(self, qa_id: int, feedback: str = None,
                                follow_up_action: str = None, resolved: bool = None):
        """Обновить фидбек на Q&A"""
        async with self.pool.acquire() as conn:
            updates = []
            params = []
            param_count = 1
            ...
            if feedback is not None:
                updates.append(f"user_feedback = ${param_count}")
                params.append(feedback)
                param_count += 1
            
            if follow_up_action is not None:
                updates.append(f"follow_up_action = ${param_count}")
                params.append(follow_up_action)
                param_count += 1
            
            if resolved is not None:
                updates.append(f"problem_resolved = ${param_count}")
    ...
    async def update_problem_solution(self, problem_id: int, solution: str,
                                     result: str = None, resolved: bool = False):
        """Обновить решение проблемы"""
        async with self.pool.acquire() as conn:
            await conn.execute("""
                UPDATE plant_problems_log
                SET solution_tried = $1,
                    solution_date = CURRENT_TIMESTAMP,
                    result = $2,
                    resolved = $3,
                    resolution_date = CASE WHEN $3 THEN CURRENT_TIMESTAMP ELSE NULL END
                WHERE id = $4
            """, solution, result, resolved, problem_id)
    ...
    async def save_user_pattern(self, plant_id: int, user_id: int, pattern_type: str,
        ...
        async with self.pool.acquire() as conn:
            # Проверяем существует ли такой паттерн
            ...
            if existing:
                # Обновляем существующий с повышением уверенности
    ...
    async def save_plant_environment(self, plant_id: int, user_id: int, **kwargs):
        """Сохранить информацию об условиях содержания"""
        async with self.pool.acquire() as conn:
            # Проверяем существует ли запись
            existing = await conn.fetchrow("""
                SELECT id FROM plant_environment WHERE plant_id = $1
            """, plant_id)
            ...
            if existing:
                ...
                if updates:
                    ...
                    query = f"""
                        UPDATE plant_environment
                        SET {', '.join(updates)}, updated_date = CURRENT_TIMESTAMP
                        WHERE plant_id = ${param_count}
                    """
                    await conn.execute(query, *params)
            else:
                ...
                await conn.execute("""
                    ...
                    (plant_id, user_id, location, lighting, humidity_level, 
                     temperature_range, air_circulation, distance_from_window, notes)
                    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                """, plant_id, user_id, 
                    kwargs.get('location'), kwargs.get('lighting'),
                    kwargs.get('humidity_level'), kwargs.get('temperature_range'),
                    kwargs.get('air_circulation'), kwargs.get('distance_from_window'),
                    kwargs.get('notes'))
